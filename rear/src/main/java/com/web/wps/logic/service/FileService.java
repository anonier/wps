...
public class FileService extends BaseService<FileEntity, String> {
    ...
    private Map<String, Object> getDbFileInfo(String userId) {
        ...
        // 获取文件信息
        FileEntity fileEntity = this.findOne(fileId);
        URL url = ossUtil.getDownLoadUrl(diskName + fileEntity.getDownload_url().substring(fileEntity.getDownload_url().lastIndexOf("/") + 1), bucketName);
        fileEntity.setDownload_url(url.toString());
    ...
    public Map<String, Object> fileVersion(int version) {
        ...
        URL url = ossUtil.getDownLoadUrl(diskName + fileInfo.getDownload_url().substring(fileInfo.getDownload_url().lastIndexOf("/") + 1), bucketName);
        fileInfo.setDownload_url(url.toString());
    ...
    public int delFile(String id) {
        ...
        if (file != null) {
            if ("Y".equalsIgnoreCase(file.getCanDelete())) {
                ...
                ossUtil.deleteFile(bucketName, diskName, file.getDownload_url().substring(file.getDownload_url().lastIndexOf("/") + 1));
                return 1;
            } else {
                return 0;
            }
